...
export const SideMenu = () => {
    ...
    return (
        <div className={`${styles.menuContainer} ${statusMenu.menu && styles.menuContainer_background}`}>
            <nav className={clsx(styles.menu, isMenuVisible && styles.menu_transition)}>
                    <header className={styles.menu__header}>
                        <img className={styles.menu__logo} src={logo} alt="меню" />

                        <button className={styles.menu__burgerMenu} onClick={() => {click()}}>
                            <MenuIcon sx={{ fontSize: 40, color: "#3D5138"}} />
                        </button>
                    </header>
                    <ul className={styles.menu__ul}>
                        <li className={styles.menu__li}>
                            <a className={styles.menu__link} href='/'>Профиль</a>
                            <ArrowForwardIcon sx={{ fontSize: 45 }}/>
                        </li>
                        <li className={styles.menu__li}>
                            <a className={styles.menu__link} href='/'>Расписание</a>
                            <ArrowForwardIcon sx={{ fontSize: 45 }}/>
                        </li>
                        <li className={`${styles.menu__li} ${styles.menu__li_padding}`}>
                            <a className={styles.menu__link} href='/'>Открыть матч</a>
                            <ContentPasteIcon sx={{ fontSize: 35 }}/>
                        </li>
                        <li className={styles.menu__li}>
                            <button className={styles.menu__buttonClose} onClick={() => {click()}}>Выйти</button>
                        </li>
                    </ul>
            </nav>
        </div>
